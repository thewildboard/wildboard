 ...
 * Manifest Class *
 ******************/
function Provider(core) {
  IndicatorController = require("../controllers/indicator_controller")(core.routes);
}
...
Provider.prototype = {
  ...
  loadSources: function(json) {
    ...
    for (var s in json.sources) {
      ...
      // determine if the source needs auth or not
      // by default it relais on the provider
      source.requires_auth = source.requires_auth || !!json.auth;
  ...
  log: function(first, second) {
    Log.log(second ? first : "", "(Provider) " + (second ? second : first));
  }
};
