...
var WidgetController = {
  create: function(db, userId, id, data, fn) {
    var self = this;
  ...
  remove: function(Widget, id, fn) {
    Widget.remove({
      _id: id
    }, fn);
  },

  removeByBoard: function(Widget, id, fn) {
    Widget.remove({
      board: id
    }, fn);
  },

  findByBoard: function(Widget, boardId, fn) {
    Widget.find({
      board: boardId
  ...
  findById: function(Widget, id, fn) {
    Widget.findOne({
      _id: id
    }, function(err, widget) {
      if (err) console.log(err);
