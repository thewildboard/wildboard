...
function Routes(_core, _server) {
   ...
   * Boards
   */
  server.get('/api/boards', requireAuth, useModel("board"), boards);
  server.post('/api/boards', requireAuth, useModel("board"), createBoard);
  server.get('/api/boards/:id', requireAuth, useModel("board"), board);
  server.patch('/api/boards/:id', requireAuth, useModel("board"), updateBoard);
  server.delete('/api/boards/:id', requireAuth, useModels(["board", "widget"]), deleteBoard);

  /**
   * Widgets
   */
  server.get('/api/boards/:id/widgets', requireAuth, useModel("widget"), widgets);
  server.post('/api/boards/:id/widgets', requireAuth, useModel("widget"), createWidget);
  server.get('/api/widgets/:id', requireAuth, useModel("widget"), widget);
  server.patch('/api/widgets/:id', requireAuth, useModel("widget"), updateWidget);
  server.delete('/api/widgets/:id', requireAuth, useModel("widget"), deleteWidget);
...
function signup(req, res) {
  ...
  UserController.signup(req.db, params.username, params.email, params.password, function(error, user) {
    if (error) {
      if (error.code == 11000) {
        return res.status(400).json({
